...
def find_boundary_worker(args: tuple) -> int:
    """경계 탐색 전용 워커 — 메인 프로세스에서 reader를 따로 만들지 않기 위해 분리"""
    pdf_path, start_idx = args
    doc = fitz.open(pdf_path)
    result = find_boundary(worker_reader, doc, start_idx)
    doc.close()
    return result
...
class PDFParser(BaseProcessor):
    ...
    def run(self):
        ...
        # ── 1) 경계 탐색 — 워커 2개로 병렬 실행 (메인 reader 생성 없음) ──
        print("=== 경계 탐색 (병렬) ===")
        with Pool(processes=2, initializer=init_worker) as pool:
            boundary_1, boundary_2 = pool.map(find_boundary_worker, [
                (self.pdf_path, a[-1]),
                (self.pdf_path, b[-1]),
            ])
